...
public class Report {
    ...
    public Report(VCFFile vcfFile, VEPAnnotationFile vepFile, HashSet<String> preferredTranscripts){
    ...
    public void writeReport(){
        ...
        try (PrintWriter writer = new PrintWriter(new File("output.txt"))) { //TODO: Print to separate SampleID files
            ...
            for (VCFRecord vcfRecord : vcfFile.getVCFRecords()){
                ...
                for (Map.Entry<String, Integer> sampleID : vcfFile.getSampleIDs().entrySet()) {
                    ...
                    //skip wildtype genotypes & low qualtiy calls
                    ...
                    if (vepFile.getTranscriptLevelRecords().containsKey(vcfRecord.getGenomeVariant())) {
    ...
    private void printGenomicAnnotation(VCFRecord vcfRecord, PrintWriter writer){
        ...
        if (vepFile.getGenomeLevelRecords().containsKey(vcfRecord.getGenomeVariant())) {

            if (vepFile.getGenomeLevelRecords().get(vcfRecord.getGenomeVariant()).isHasAaMAF()) {
                writer.print(vepFile.getGenomeLevelRecords().get(vcfRecord.getGenomeVariant()).getAaMAF() + "\t");
            } else {
                writer.print("\t");
            }

            if (vepFile.getGenomeLevelRecords().get(vcfRecord.getGenomeVariant()).isHasEaMAF()) {
                writer.print(vepFile.getGenomeLevelRecords().get(vcfRecord.getGenomeVariant()).getEaMAF() + "\t");
            } else {
                writer.print("\t");
            }

            if (vepFile.getGenomeLevelRecords().get(vcfRecord.getGenomeVariant()).isHasAfrMAF()) {
                writer.print(vepFile.getGenomeLevelRecords().get(vcfRecord.getGenomeVariant()).getAfrMAF() + "\t");
            } else {
                writer.print("\t");
            }

            if (vepFile.getGenomeLevelRecords().get(vcfRecord.getGenomeVariant()).isHasAmrMAF()) {
                writer.print(vepFile.getGenomeLevelRecords().get(vcfRecord.getGenomeVariant()).getAmrMAF() + "\t");
            } else {
                writer.print("\t");
            }

            if (vepFile.getGenomeLevelRecords().get(vcfRecord.getGenomeVariant()).isHasAsnMAF()) {
                writer.print(vepFile.getGenomeLevelRecords().get(vcfRecord.getGenomeVariant()).getAsnMAF() + "\t");
            } else {
                writer.print("\t");
            }

            if (vepFile.getGenomeLevelRecords().get(vcfRecord.getGenomeVariant()).isHasEurMAF()) {
                writer.print(vepFile.getGenomeLevelRecords().get(vcfRecord.getGenomeVariant()).getEurMAF() + "\t");
            } else {
                writer.print("\t");
            }
        } else {
